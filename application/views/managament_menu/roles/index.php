<div class="row">
    <div class="col-md-12 col-sm-12 col-xs-12">
      <div class="x_panel">
        ...
        <div class="x_content">
           <table id="tblRoles" class="table table-striped jambo_table bulk_action">
             <thead>
               <tr clas="headings">
                 <th name='No'>No</th>
                 <th>Nama Role</th>
...
<script>
  ...
  var tabelRoles = $('#tblRoles').DataTable({
         "processing": true,
         "ordering" : true,
         "serverSide": true,
         "ajax": {
    ...
    function get_detail(data,btn){ 
      ...
      $('.modal-body').load(module_url + "/getForm",function(){
        ...
        $.ajax({
          ...
          type: "POST",
          dataType: 'JSON',
          data : { 'id_roles_menu' : data, <?= $csrf['name']; ?> : csrf_token },
    ...
    function deleteData(data, btn) {
        let dataDel = { 'id_roles_menu' : data, 'roket' : csrf_token };
        let url = '/delete';
        let button = $(btn).html();
    ...
    function updateData(data, btn){
      ...
      let formData = $('#form-data').serializeArray();
      formData.push({name : 'id_roles_menu',value : data});
